...
export default function AddQuestionModal({ topicId, onClose }) {
  ...
  const handleSubmit = (e) => { e.preventDefault(); if (form.title.trim()) addQuestion.mutate(form); };
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 mb-5">
          ...
          <h3 className="text-base font-bold" style={{ color: t.textPrimary }}>Add Question</h3>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input autoFocus type="text" value={form.title} onChange={set('title')} placeholder="e.g. Two Sum"
          ...
          <div className="flex gap-2.5 justify-end pt-1">
            ...
            >Cancel</button>
            <button type="submit" disabled={!form.title.trim() || addQuestion.isPending}
            ...
            >{addQuestion.isPending ? '...' : 'Add Question'}</button>
          </div>
        </form>
      </div>
    </div>
